...
class loginWithSameUser extends Simulation {
  ...
  val feeder = csv("login.csv").queue
  val queueSize = feeder.records
  println("Processing " + queueSize.toString + " records from login.csv file ")
  ...
  object Login {
    ...
    val scn1 =
      feed(feeder).exec(http("login - single user") // Here's an example of a POST request
        .post("/api/login")
        .headers(headers_1)
